...
#include <Adafruit_ILI9341.h> // or your specific TFT library
...
int activeButton = 1; // 0: None, 1: B1, 2: B2, 3: B3
...
void drawColorPickerView() {
  ...
  for (int i = 0; i < 256; i += colorStep) {
    for (int j = 0; j < 256; j += colorStep) {
      sprite.fillRect(STICKY_BAR_HEIGHT + i / 2, j, colorStep, colorStep, tft.color565(i, j, 255 - i));
    }
  }
}
...
void handleTouch(int x, int y) {
  ...
  if (transformedX < STICKY_BAR_HEIGHT) {
    ...
    } else if (transformedY >= BUTTON_SIZE + BUTTON_MARGIN && transformedY < 2 * BUTTON_SIZE + BUTTON_MARGIN) { // Button B2
      activeButton = (activeButton == 2) ? 0 : 2; // Toggle active state
    } else if (transformedY >= 2 * BUTTON_SIZE + 2 * BUTTON_MARGIN && transformedY < 3 * BUTTON_SIZE + 2 * BUTTON_MARGIN) { // Button B3
...
void loop() {
  ...
  if (touch.read()) {
    ...
    // Apply momentum and friction if not actively touching and if velocity is significant
